...
const findAdapter = (
    ...
    if (!hasLibraryScan(adapter)) {
        ...
        throw new ServiceError('NotFound', service, `${service} has no library to scan`, {
            remedy: 'Only radarr, sonarr, jellyfin and plex manage a library, and prowlarr syncs its indexers to them. For "it downloaded but will not play", the media server — jellyfin or plex — is almost always the one you want.'
        });
    }
...
export function registerTriggerScan(
    ...
    registerWriteTool(server, context, {
                    ...
                    effects: [
                        ...
                        rotation
                            ...
                            : chain
                              ...
                              : 'Renames only the files above afterwards, so the filenames match the episodes they have been moved to. Nothing else in the series is renamed, and the naming format is not touched.',
        ...
        async apply(_plan, { service, instance, action, id, download_id, reassignments }) {
            ...
            }${handle.detail === undefined ? '' : ` ${handle.detail}`}`;
        }
    });
}

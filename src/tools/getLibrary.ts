        ...
        'get_library',
        {
            ...
            outputSchema: PagedOutputSchema.extend({
                note: z
                    .string()
                    .optional()
                    .describe('Present when a fact about the stack\'s config, not this call, needs stating — no media server configured at all, or one configured with no default_user.')
            }),
            inputSchema: toolInput({
                ...
                watched: z.boolean().optional().describe('The media server\'s watch state. Items it has never seen count as unwatched. Pair with `user` to ask about someone else.'),
                user: UserSchema,

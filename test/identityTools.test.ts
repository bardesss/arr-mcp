...
const jellyfinRoutes = {
    '/Users': USERS,
    '/Sessions': SESSIONS,
    [`/Items?userId=${USER_ID}&Recursive=true&IsResumable=true&IncludeItemTypes=Movie,Episode&EnableUserData=true`]: RESUME
};
...
describe('get_playback', () => {
    ...
    it('omits the percentage rather than dividing by zero when runtime is unknown', async () => {
        ...
        const { adapter, resolver } = jellyfin({}, { ...jellyfinRoutes, [`/Items?userId=${USER_ID}&Recursive=true&IsResumable=true&IncludeItemTypes=Movie,Episode&EnableUserData=true`]: noRuntime });
        const result = await buildGetPlayback(adapter, resolver, { detail: 'full', limit: 50 });
    ...
    it('permits another user when allow_other_users is true', async () => {
        const guestRoutes = { ...jellyfinRoutes, [`/Items?userId=${GUEST_ID}&Recursive=true&IsResumable=true&IncludeItemTypes=Movie,Episode&EnableUserData=true`]: { Items: [] } };
    ...
    it('reports truncation honestly', async () => {
        ...
        const { adapter, resolver } = jellyfin({}, { ...jellyfinRoutes, [`/Items?userId=${USER_ID}&Recursive=true&IsResumable=true&IncludeItemTypes=Movie,Episode&EnableUserData=true`]: many });
        const result = await buildGetPlayback(adapter, resolver, { detail: 'standard', limit: 50 });
